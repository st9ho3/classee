...
export const updateClasses = (newClass) => {
   ...
   const wantedUser = users.filter((user) => user.Email === authUser.Email && user.Password === authUser.Password)[0]
   const newUsers = users.filter((user) => user.Email !== authUser.Email || user.Password !== authUser.Password)
